...
@section('document.body')
  <div class="max-w-2xl mx-auto my-8 lg:my-12 px-4">
    <div class="space-y-6 mb-6">
      ...
      <div class="text-sm breadcrumbs py-0">
        <ul>
          ...
          <li class="text-base-content/60">Buat Blog</li>
        </ul>
      </div>
    ...
    <form method="POST" action="{{ route('guest.blog.kelola.store') }}" enctype="multipart/form-data" class="space-y-6">
      @csrf
      <div>
      ...
      <div>
        ...
        <div id="quill-editor" style="min-height: 200px;">{!! old('body') !!}</div>
      ...
      <div>
        ...
        <input type="number" name="read_duration" class="input input-bordered w-full" min="1" max="60"
          value="{{ old('read_duration') }}" required>
      </div>
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          @foreach ($categories as $cat)
            <label class="cursor-pointer flex items-center gap-2">
              <input type="checkbox" name="categories[]" value="{{ $cat->id }}" class="checkbox"
                @if (collect(old('categories'))->contains($cat->id)) checked @endif>
      ...
      <div>
        <label class="label font-semibold">Gambar</label>
        <input type="file" name="primary_image" class="file-input file-input-bordered w-full" accept="image/*"
          required>
        <span class="text-xs text-base-content/60">Format file yang didukung: png, jpg, jpeg. Ukuran maksimal 2 MB.</span>
      </div>
